...
return this.products.filter(p=>p.CategoryId==categoryid)
  ...
  getnextproductid(currentid:number):number{
    console.log("kfj")
  let index=this.products.findIndex((a)=>a.id===currentid)
  const findedproduct=this.products[++index]
  console.log('Next Product ID:', findedproduct.id);
  return findedproduct.id;
  ...
  getpreviousproduct(currentid:number):number{
    let index=this.products.findIndex((a)=>a.id===currentid)
    const pre=this.products[--index]
    return pre.id;
  }
  ...
  islastindex(id:number|undefined){
   ...
   isfirst(id:number|undefined){
     if(!id)return false
      let findedproduct=this.products[0];
      return findedproduct.id==id;
    }
}
